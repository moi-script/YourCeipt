        ...
        <div className="flex justify-between items-end">
             <div>
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800 backdrop-blur-sm shadow-sm mb-2">
                    ...
                    <span className="text-[10px] uppercase tracking-widest text-emerald-700 dark:text-emerald-400 font-bold">Ledger</span>
                </div>
                <h1 className="text-4xl font-serif italic text-[#2c2c2c] dark:text-stone-100">Transaction Flow</h1>
             </div>
             <Button 
          ...
          {filteredTransactions.length === 0 ? (
            <Card className="border-none bg-white/40 dark:bg-stone-900/40 shadow-none rounded-[2rem]">
              <CardContent className="p-12 text-center text-stone-500 dark:text-stone-400">
                ...
                <p className="font-serif italic text-lg">No transactions found</p>
              </CardContent>
            </Card>
          ) : (
            filteredTransactions.map((t) => {
              ...
              return (
                <Card 
                    key={t.id} 
                    className={`border-white/50 dark:border-stone-800 bg-white/70 dark:bg-stone-900/70 backdrop-blur-md shadow-[0_4px_20px_rgb(0,0,0,0.02)] rounded-[2rem] overflow-hidden transition-all duration-300 ${isOpen ? 'ring-2 ring-emerald-100 dark:ring-emerald-900/50 shadow-md' : 'hover:bg-white/90 dark:hover:bg-stone-800/80'}`}
                >
                  {/* COLLAPSED HEADER */}
                  <CardHeader
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-serif text-lg text-stone-800 dark:text-stone-100">{t.store}</p>
                    ...
                    <div className="flex items-center gap-4">
                      <p className={`text-lg font-bold ${t.type === 'income' ? 'text-emerald-700 dark:text-emerald-400' : 'text-stone-800 dark:text-stone-200'}`}>
                        {t.type === 'expense' && '-'} {t.metadata.currency} {t.total.toFixed(2)}
                      </p>
                      <div className={`p-2 rounded-full transition-colors ${isOpen ? 'bg-stone-100 dark:bg-stone-800 text-stone-800 dark:text-stone-200' : 'text-stone-400 dark:text-stone-500'}`}>
                    ...
                    <CardContent className="space-y-6 p-6 pt-0">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-stone-50/50 dark:bg-stone-800/40 p-6 rounded-[1.5rem]">
                          <div className="space-y-1">
                             <p className="text-xs font-bold uppercase tracking-wider text-stone-400 dark:text-stone-500">Location</p>
                             <p className="text-sm text-stone-700 dark:text-stone-300 font-medium">
                                 {t.address.street}, {t.address.city}
                             </p>
                             <p className="text-xs text-stone-500 dark:text-stone-400">{t.address.state} {t.address.zip}</p>
                          </div>
                          <div className="space-y-1">
                             <p className="text-xs font-bold uppercase tracking-wider text-stone-400 dark:text-stone-500">Details</p>
                             <div className="flex gap-4 text-xs text-stone-600 dark:text-stone-300">
                                 <div>
                                     <span className="text-stone-400 dark:text-stone-500">Store #:</span> {t.transaction.store_number}
                                 </div>
                                 <div>
                                     <span className="text-stone-400 dark:text-stone-500">Term:</span> {t.transaction.terminal_number}
                                 </div>
                             </div>
                             <p className="text-xs text-stone-500 dark:text-stone-400">Mgr: {t.manager || "N/A"}</p>
                          </div>
                      </div>

                      {/* ITEMS */}
                      <div className="space-y-3">
                        <p className="text-xs font-bold uppercase tracking-wider text-stone-400 dark:text-stone-500 ml-2">Receipt Items</p>
                        <div className="space-y-2">
                            {t.items.map((item, idx) => (
                              ...
                              >
                                <div className="flex gap-3">
                                    <span className="bg-stone-100 dark:bg-stone-800 text-stone-500 dark:text-stone-400 px-2 rounded-lg text-xs flex items-center">x{item.quantity}</span>
                                    <div className="flex flex-col">
                                        <span className="font-medium text-stone-700 dark:text-stone-200">{item.description}</span>
                                        <span className="text-[10px] text-stone-400 dark:text-stone-500 uppercase tracking-wide">{item.type}</span>
                                    </div>
                                </div>
